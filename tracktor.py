...
def gray_otsu_thresh(frame, block_size = 31, offset = 25):
    """
    This function retrieves a video frame and preprocesses it for object tracking.
    The code blurs image to reduce noise, converts it to greyscale and then returns a 
    thresholded version of the original image.
    ...
    """
    frame = cv2.blur(frame, (5,5))
    ret3,thresh = cv2.threshold(frame, 0, 255,cv.THRESH_BINARY+cv.THRESH_OTSU)
...
def detect_and_draw_contours(frame, thresh, meas_last, meas_now, min_area = 0, max_area = 10000, min_circularity = 0.5):
    ...
    while i < len(contours):
        ...
        if area < min_area or area > max_area or circularity < min_circularity:
